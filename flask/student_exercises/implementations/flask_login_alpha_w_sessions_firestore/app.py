...
def register():
    if request.method == "POST":
        ...
        # TODO: insert data in firestore database, using the firestore automatic id generator
...
def login():
    if request.method == "POST":
        ...
        # TODO: get all the users from the firestore users collection database
        with open(CSV_FILE, "r") as csv_file:
            csv_reader = csv.DictReader(csv_file)
            # Loop throught the users from firestore database and check if email/password match
            for row in csv_reader:
                # TODO: check email
                if row["email"] == email:
...
def private() -> str:
    ...
    # TODO: Display user info (firstname, lastname, email) in the web page private.html
