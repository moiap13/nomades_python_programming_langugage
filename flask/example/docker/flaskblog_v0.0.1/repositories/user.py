...
class UserRepository():
  ...
  def create(self, user: User) -> User:
    _, user_doc_ref = self.collection_ref.add(user.to_dict(include_pwd=True))
    user.id = user_doc_ref.id
    return user

  def get_user_ref(self, user_firestore_id) -> "DocumentReference":
    return self.collection_ref.document(user_firestore_id)
  ...
  def get_by_fullname(self, firstname: str, lastname: str) -> User:
    user_snapshots = self.collection_ref.where("firstname", "==", firstname).where("lastname", "==", lastname).get()

    if len(user_snapshots) == 0:
      raise UserNotFoundError(f"No users found with fullname={firstname} {lastname}")
