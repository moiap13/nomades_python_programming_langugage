...
@user_bp.route("/info/<firestore_id>")
@authenticated
def user_info_firestore(firestore_id: str) -> str:
    # try:
    #     user: dict[str, str] = get_user_by_uid(session["uid"], db)
    # except UserNotFoundError:
    #     flash("User not found", "danger")
    ...
    user: dict[str, str | int] = (
        db.collection("users").document(firestore_id).get().to_dict()
    )
    img_path = url_for(
    ...
    return render_template("user_wtf/userinfo.html", user=user, allow_modify=False)
