...
def brute_force_password(charset: str, length: int, target_hash: str):
    ...
    # charset_l: list[str] = list(charset)
    # random.shuffle(charset_l)
    # charset = "".join(charset_l)
    print(charset)    

    start_time = time.time()

    # for c1 in charset: # 26
    #   for c2 in charset: # 26 * 26
    #     for c3 in charset: # 26^3
    #       for c4 in charset: # 26^4
    #         for c5 in charset: # 26^5
    #           p: str = f"{c1}{c2}{c3}{c4}{c5}"
    ...
    for i in tqdm.tqdm(range(MAX_ITER)):
      pwd = ""
      for p in range(length-1, -1, -1): # p loops from 4 to 0 both included
        pwd += charset[i // len(charset)**p % len(charset)]
      password: str = hashlib.sha256(pwd.encode()).hexdigest()
      if password == target_hash:
        # you found the password hourra
